...
>
> **Actualización (viabilidad del sensor de torque, 28/08/2026):** en la misma reunión, los profesores guía objetaron por separado la implementación del **sensor de torque** descrita en §2.5 (celda de carga fija sobre brazo de palanca): no permite acompañar el cambio de ángulo de la aleta, y sugirieron evaluar un torquímetro o strain gauges como alternativa (acuerdo #4, `00_Administración/02_Registro_Reuniones_Avance.md`). Esta es una reevaluación distinta de la del párrafo anterior (esa es sobre el motor/tecnología de aplicación de torque; esta es sobre el sensor de medición). Ver nota específica en §2.5.
...
- **Implementación mecánica del acople (preliminar):** motor de carga y actuador se unen mediante un **eje intermedio de 5 mm de diámetro**, conectado a cada servo con **acoples flexibles de aluminio 5 mm** (toleran pequeños desalineamientos entre los cuernos/horns de los servos y el eje). El eje se apoya en **dos rodamientos tipo pillow block** montados en el riel de la bancada. La **aleta** se fija al eje mediante un collar cerca del extremo del actuador. El **sensor de torque no es inline**: se implementa como una **celda de carga fija al riel** (modelo Altronics LC-10KG-HX711, tipo barra, 10 kg, con módulo HX711 de 24 bits), presionada por un **brazo de palanca (torque arm) de radio ~4 cm** rígidamente sujeto al eje (torque = fuerza medida × radio; ver RNF-CAR-01, RNF-PRE-02). Las piezas custom (collares, brazo de torque, soportes de rodamiento) se fabrican por **impresión 3D**. **Se incorporarán dos switches de fin de carrera físicos** (uno por sentido de giro) montados en la bancada, accionados por un lóbulo o leva solidaria al eje, calibrados para activarse ligeramente antes de que la aleta alcance los límites angulares máximos que el actuador bajo prueba puede físicamente comandar — siguiendo el enfoque de Anastasopoulos & Hornung (2018). Estos switches interrumpen directamente la alimentación de potencia del driver del motor de carga (no solo la señal de control), actuando como protección de último nivel independiente del software (RF-BAN-08).

  > **⚠️ Nota (28/08/2026, en reevaluación):** esta implementación del sensor de torque (celda de carga fija + brazo de palanca) fue objetada por los profesores guía: no permite acompañar el cambio de ángulo de la aleta durante el ensayo, ya que el brazo asume un radio efectivo fijo respecto a la celda. Se sugirió evaluar como alternativa un **torquímetro** (posiblemente inline, rotativo) o **strain gauges** montados directamente sobre el eje (acuerdo #4, `00_Administración/02_Registro_Reuniones_Avance.md`). Esta reevaluación es independiente de la del motor de carga (ver nota general al inicio del documento) y afecta directamente RNF-CAR-01, RNF-PRE-02 y RNF-PRE-04. Mientras no se cierre, la descripción de esta sección sigue siendo la arquitectura de referencia, no una decisión definitiva. **La incorporación de los fin de carrera no depende de esta reevaluación** — su posición se define respecto al rango angular del actuador bajo prueba, no respecto a la tecnología de sensor de torque.
  >
...
> **Nota (reevaluación en curso — motor de carga):** la elección de motor DC crudo con driver H-bridge simple, descrita arriba, es la arquitectura de referencia actual pero está siendo comparada formalmente contra un motor brushless y una arquitectura lineal — ver `Comparacion_Alternativas_Arquitectura_Fisica.md`.
...
8. **Alineación mecánica y margen dinámico del conjunto en un solo eje:** el montaje en serie (motor de carga–sensor–actuador–aleta) exige alta concentricidad para que el sensor de torque no registre cargas parásitas radiales o de flexión; adicionalmente, la frecuencia natural del conjunto (incluyendo la inercia de la aleta) debe quedar suficientemente alejada del ancho de banda del lazo de control para evitar acoplamiento dinámico no deseado (ver RNF-PRE-04 y RNF-REN-03). **Este supuesto asume la geometría de sensor de torque actualmente en reevaluación (§2.5); debe revisarse si cambia la tecnología de medición.**
...
- ~~Selección de celda de carga concreta~~ — **En reevaluación (28/08/2026):** se había dado por completada la selección de la celda de carga tipo barra Altronics LC-10KG-HX711 (10 kg, con módulo HX711), sobre el brazo de palanca de ~4 cm (ver `06_Seleccion_Actuador_de_Carga.md`, §6.2, y RNF-PRE-02), pero los profesores guía objetaron esta implementación por no acompañar el cambio de ángulo de la aleta (acuerdo #4). Queda pendiente evaluar alternativas (torquímetro, strain gauges) antes de dar este punto por cerrado — ver nota en §2.5.
...
- ~~Propagación pendiente de los acuerdos de Avance I a RNF, CU y geometría CFD~~ — **Completado** en `03_Requisitos_No_Funcionales.md`, `04_Casos_de_Uso.md` (CU-010) y este documento (§2.1). Pendiente aún: propagar la confirmación de AoA/deflexión separados (§2.1) a `04_CFD/01_Casos/01_Geometria_Aleta_Referencia.md`, que sigue registrando la pregunta como abierta.
...
- **(Nuevo, 28/08/2026) Reevaluación del sensor de torque:** ver nota en §2.5. Pendiente evaluar torquímetro o strain gauges como alternativa a la celda de carga + brazo de palanca actual, y actualizar §2.5, §5 (supuesto 8) y las interfaces I-04/I-05 si cambia la tecnología.
