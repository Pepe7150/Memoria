...
**Nota de estado (28/08/2026):** los valores numéricos propuestos aquí (RNF-REN-01/02, RNF-PRE-01/02/03) son objetivos de desempeño independientes de la tecnología de sensor concreta, y no se ven afectados por la reevaluación del sensor de torque acordada con los profesores guía (acuerdo #4, `00_Administración/02_Registro_Reuniones_Avance.md`). Lo que sí está en reevaluación es la implementación específica (celda de carga + brazo de palanca) documentada en `06_Seleccion_Actuador_de_Carga.md` §6.2 y `05_Arquitectura_del_Sistema.md` §2.5 — ver esos documentos para el estado actual. El valor objetivo de RNF-PRE-02 (≤0,5% FE) sigue siendo válido como meta, cualquiera sea el sensor finalmente elegido.
...
| RNF-PRE-02 | Incertidumbre del sistema de medición de torque                      | **≤ 0,5 % del fondo de escala** (referencia óptima observada: 0,1 % FE)                | Basado en las tres clases de sensor de torque reportadas por Anastasopoulos & Hornung (2018); se propone un valor algo más conservador que el óptimo reportado (0,1 %) para dar margen a componentes COTS de menor costo, alineado con RNF-COS-01.**Nota (28/08/2026): este valor objetivo (≤0,5% FE) es independiente de la tecnología de sensor; no se ve afectado por la reevaluación de la implementación celda+brazo (ver nota de estado al inicio de este documento).** |
...
- `03_Requisitos_No_Funcionales.md`: reemplazar los campos "[a definir]" de RNF-REN-01, RNF-REN-02, RNF-PRE-01, RNF-PRE-02 y RNF-PRE-03 por los valores preliminares de la sección 4, dejando explícita su naturaleza provisional (p. ej. "[preliminar, ver `07_Valores_Referencia_Literatura_Analoga.md`]"). **Ya incorporado**, junto con la nota de reevaluación del sensor de torque (acuerdo #4, 28/08/2026) en RNF-CAR-01/RNF-PRE-02.
...
Con estos valores ya disponibles, corresponde continuar con el **Paso 2** (actualizar `03_Requisitos_No_Funcionales.md` con estos valores preliminares) — **ya completado** — y, en paralelo o a continuación, el **Paso 3** (actualizar la interfaz I-03 en `05_Arquitectura_del_Sistema.md`) — **ya completado** —, quedando el **Paso 4** (preselección de modelos COTS concretos de motor/reductor/sensor) — **completado para motor/driver/microcontrolador (ver `06_Seleccion_Actuador_de_Carga.md` §6.1); en reevaluación para el sensor de torque específico (§6.2 del mismo documento, acuerdo #4)** — como el pendiente real restante de esta secuencia.
