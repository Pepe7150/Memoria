...
> **Actualización (27–28/08/2026):** a solicitud de los profesores guía, la arquitectura de aplicación de torque aquí documentada (§4) está en **reevaluación activa** frente a dos alternativas adicionales: motor brushless (BLDC/PMSM) y actuador lineal + brazo de palanca. Ver `Comparacion_Alternativas_Arquitectura_Fisica.md` (comparación cualitativa) y `Matriz_Decision_Arquitectura_Banco.xlsx` (matriz de decisión ponderada, hoja "Torque - Motor de Carga"). **La decisión de §4 sigue vigente como arquitectura de referencia** mientras no se cierre esta reevaluación; ningún componente ya seleccionado (DS3218, MG996R, BTS7960, INA219, celda de carga) se ha descartado.
...
**Nota (reevaluación 27–28/08/2026):** los profesores guía reabrieron la posibilidad de una arquitectura lineal, pero con un actuador distinto del L16-P (p. ej. tipo voice coil, de mayor ancho de banda) — no es la misma alternativa aquí descartada. Se documenta como Opción C en `Comparacion_Alternativas_Arquitectura_Fisica.md`, con dos sub-variantes de medición (C1/C2). El motivo de descarte de esta sección (velocidad insuficiente del L16-P) no aplica automáticamente a C — debe evaluarse con el actuador lineal específico que se proponga.
...
### 4.3 Estado de la reevaluación (27–28/08/2026)

Esta decisión está bajo reevaluación formal a solicitud de los profesores guía, motivada por la objeción de que "en motores DC no se controla el torque directamente" (aclarada y reencuadrada en `Comparacion_Alternativas_Arquitectura_Fisica.md` §0: el problema real identificado es la fricción/backlash de la reductora del DS3218, no una limitación de controlar torque vía corriente en motores DC per se — ver diagrama del lazo de corriente en la misma sección). Se evalúan tres alternativas (motor DC actual, brushless, lineal+brazo) en una matriz de decisión ponderada de 12 criterios (`Matriz_Decision_Arquitectura_Banco.xlsx`). **Resultado preliminar de la matriz (puntajes de partida, sujetos a ajuste con los profesores):** la arquitectura actual (A) queda mejor posicionada, principalmente por reutilización de trabajo ya avanzado, bajo riesgo de cronograma y compatibilidad con la tolerancia de alineación ya modelada — pero el ancho de banda dinámico real (§4.2, tercer punto) sigue sin validarse experimentalmente en ninguna alternativa, y es el criterio que más pesa en la comparación.
...
3. **Caracterizar el motor DC crudo** dentro del DS3218 una vez intervenido (corriente nominal/de stall, curva torque–velocidad, constante eléctrica) — **avance parcial:** los datasheets oficiales a nivel de servo completo ya están documentados y citados en `08_Datasheet_Motor_DS3218.md` (confirman ~1,8 A / ~2,11 N·m de bloqueo a 6,8 V para la variante estándar DS3218MG, y advierten sobre una variante PRO con ~2,9 A). El motor DC interno en sí, sin embargo, no tiene datasheet propio publicado (componente genérico sin marca); su caracterización eléctrica (Kt, resistencia de armadura, curva torque-velocidad aislada) sigue requiriendo medición experimental una vez intervenido el servo.
...
6. **Verificar resistencia mecánica** del brazo de torque y los collares impresos en 3D frente al torque pico (~2 N·m), antes de la validación experimental. **Completado (primera pasada analítica)** en `09_Verificacion_Mecanica_Brazo_Torque.md`: brazo con sección cuadrada 14×14 mm en PLA (infill ≥80%, orientación de impresión alineada con la flexión); collares fijados al eje mediante prisionero, con plana en el eje + inserto roscado metálico (la fricción pura de un prisionero sobre eje liso resultó ~20× insuficiente para el torque pico). Pendiente: FEA sobre CAD real, dimensionamiento concreto de la plana/inserto, y ensayo físico de confirmación una vez impresas las piezas.
...
9. **Cerrar la reevaluación de arquitectura física** (27–28/08/2026) con los profesores guía, usando `Comparacion_Alternativas_Arquitectura_Fisica.md` y `Matriz_Decision_Arquitectura_Banco.xlsx` como insumo. El ítem 5 de esta lista (confirmar estrategia de control de torque) queda subsumido en esta reevaluación más amplia.
